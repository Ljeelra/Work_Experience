    ...
    while (true) {
        try{
            ...
            $('tbody tr').each((index, element) => {
                ...
                const href = $(element).find('td a').attr('href'); // href 속성 값을 가져옵니다.
                const idMatch = href.match(/announcement=(\d+)/); // 정규 표현식으로 seq 값을 추출합니다.

                if (idMatch) {
                    const idValue = idMatch[1]; // 추출된 seq 값을 가져옵니다.
