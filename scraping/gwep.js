...
const chunkSize = 50;
const baseUrl = 'http://gwep.or.kr/bbs/board.php?bo_table=gw_sub21&page=';
const detailBaseUrl = 'http://gwep.or.kr/bbs/board.php?bo_table=gw_sub21&wr_id=';
const chunkSize2 = 10;
...
async function getListPathIds(){
    ...
    while(scraping){
        try{
            ...
            const listHtml = await axiosInstance.get(baseUrl, {params: {page}});
            ...
            const dataText = $(element).find('td.td_datetime.lview').text().trim();
            ...
            //a태그 text 값에 (~M.dd)가 있고 오늘 날짜랑 비교해서 이전이면 스크랩할 필요 없음.
                const href = $(element).find('td.td_subject > div.bo_tit > a').eq(1).attr('href');
                //console.log('href값',href);
                const regex = /wr_id=(\d+)/;
                const match = href.match(regex);
                //console.log('match:',match);

                const linkText = $(element).find('td.td_subject > div.bo_tit > a').eq(1).text().trim();
                ...
                if (linkText.includes('모집마감') || linkText.includes('선정')) {
...
async function scrapeDetailPage(pathId, siteName){
    ...
    try{
        ...
        data.category = $('#bo_v_title').find('span.bo_v_cate').text().trim();
        data.announcementDate =  $('#bo_v_info').find('strong.if_date').text().trim();
        const parts = data.announcementDate.split(' ');
        data.announcementDate = parts.length > 1 ? parts.slice(1).join(' ').trim() : data.announcementDate;
        //console.log('공고일: ',data.announcementDate);

        const imgbox = $('section#bo_v_atc div#bo_v_con');
        ...
        if (imgTags.length > 0) {
            ...
            imgTags.each((index, element) => {
                ...
                if (imgSrc) {
                    ...
                    if (base64Match) {
                        const imageDir = path.join(__dirname, 'images', 'gwepimages'); // 'images/gwimages' 폴더 경로 설정
                        ...
                        const fullImgSrc = imgSrc.startsWith('/') ? `http://gwep.or.kr${imgSrc}` : imgSrc;
        ...
        file.each((index, element) => {
            ...
            const regex = /no=(\d+)/;
            const amatch = href.match(regex);
            if(amatch){
                const fileno = amatch[1].trim();
                const fileUrl = `http://gwep.or.kr/bbs/download.php?bo_table=gw_sub21&wr_id=${pathId}&no=${fileno}`;
